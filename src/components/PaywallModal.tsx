...
export function PaywallModal({ onSelectPlan, userId, userEmail, onCheckoutSuccess }: PaywallModalProps) {
  ...
  const handleContinue = async (promotionCode?: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // If promotion code was applied successfully
        if (promotionCode && data.discountApplied) {
  ...
  // Apply coupon code - recreates subscription with promotion code
  const handleApplyCoupon = async () => {
    ...
    // Only yearly plan gets the coupon
    if (selectedPlan !== 'yearly') {
      setCouponError('Coupon only valid for yearly plan');
      return;
    }
    ...
    try {
      // Recreate subscription with promotion code
      const priceId = STRIPE_PRICES.yearly;
      const response = await fetch(getEndpointUrl('createSubscription'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
        ...
        <div className="relative p-4 sm:p-6">
          ...
          <div className="mt-3 space-y-2">
            ...
            <>
                  ...
                  <p className="text-cosmic-400 text-xs">
                    {couponApplied ? (
                      <span className="text-green-400">
                        Yearly plan • <span className="line-through text-cosmic-500">$49.99</span> $0 today, then $49.99/year
                      </span>
                    ) : (
              ...
              {/* Coupon Code Input - Only for yearly plan */}
              {selectedPlan === 'yearly' && (
                <div className="mb-4">
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 rounded-lg bg-white/5 border border-white/10
                               text-white text-sm placeholder-cosmic-500
                               focus:outline-none focus:border-nebula-purple focus:ring-1 focus:ring-nebula-purple
                  ...
                  {couponApplied && (
                    <p className="mt-1 text-xs text-green-400">
                      30-day free trial applied! You'll be charged $49.99/year after the trial.
                    </p>
                  )}
                </div>
              )}
